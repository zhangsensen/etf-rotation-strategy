...
从 49 只 A 股/QDII ETF 中，每 5 个交易日自动选出 2 只持有。核心流程：

1. **因子打分** — 23 个技术/资金流因子对 ETF 截面排序
2. **迟滞过滤** — 防止频繁换仓（rank 差值 ≥ 10% 且持仓 ≥ 9 天才允许换）
3. **波动率门控** — 高波环境自动降仓位（100% → 70% → 40% → 10%）
4. **三层验证** — WFO 粗筛 → VEC 向量化回测 → BT 事件驱动审计，逐层收紧

不依赖择时、不做日内交易、不加杠杆。纯截面选股 + 执行纪律。
...
**为什么 ETF**: 个股有退市、暴雷、流动性陷阱等不可控风险；ETF 天然分散、费率低、T+1 可交易、流动性充足。49 只 ETF 覆盖了沪深主要宽基、行业和主题，足以捕获截面 alpha。

**为什么轮动而非持有**: A 股 ETF 截面收益由单一因子主导 (PC1 = 59.8%)，板块轮动特征显著。截面选股策略在 A 股 ETF 市场有天然优势——只要选对 2 只 ETF 持有 5 天，就能持续吃到板块轮动红利。

**为什么不做择时**: 实证显示执行优化的回报乘数 (3.6x) 远超信号改进 (1.25x)。波动率门控 (Regime Gate) 本质是风控而非择时——它不预测方向，只在极端波动时降低暴露。

**A 股特色约束**:
- T+1 执行模型 (信号 T 日收盘后产生，T+1 开盘执行)
- 整手约束 (100 股 / 手)
- 分市场成本 (A 股 2bp, QDII 5bp)
- QDII 硬阻断 (监控但不交易，避免外汇额度和 T+2 结算问题)
